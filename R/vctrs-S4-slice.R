...
method(bioc_slice, class_vctrs) <- function(
  x,
  i,
  ...,
...
method(bioc_slice, S7::class_any) <- function(x, i, ...) {
  x[i]
}

method(bioc_slice, class_s4_vctrs) <- function(x, i, ...) {
  x[i]
}

method(bioc_slice, class_DF) <- function(x, i, ...) {
  x@listData <- purrr::map(x@listData, bioc_slice, i = i)
  x@nrows <- length(i)
...
method(vec_chop2, class_s4_vctrs) <- function(x, ..., indices = NULL) {
  fun <- method(bioc_slice, object = x)
  if (is.null(indices)) {
    indices <- seq_along(x)
  }
