...
new_view_spec <- function(ctx, from, ...,
                     mapper = ~ .x,
...
link_view <- function(bm, .view) {
  ...
  # setup map function
  env_from$.map_fn <- new_function(pairlist2(.x =), f_rhs(.view$mapper),
                                   env = env_from)
  env_bind_lazy(
    env_from,
    !!! lapply(.view$installs,
               new_quosure,
               env = env_from)
  )
  ...
  env_bind_lazy(
    ...
    .__cache_map__. = !!new_quosure(
      expr({
        .res <- lapply(seq_len(.subset2(`biocmask:::ctx:::n_groups`, !!.view$from)), .map_fn)
  ...
  asis_lazy_bind <- biocmask:::add_bind(
    .expr = substitute(lapply(!!name_sym, \(.x) .y),
  ...
  reshape_lazy_bind <- biocmask:::add_bind(
    ...
    .expr = substitute(.subset(!!name_sym, .__uniq_indx__.) |> 
                         Map(\(.x, .i) .y, .x = _,
...
#'             # default is fine
#'             asis = ~ .subset2(.x, 1L),
#'             reshape = ~ vec_rep(.x, times = .ncols[.i])
#'           ))
#' V$bind_new("length")
#'
